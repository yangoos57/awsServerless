## 도도모아 프로젝트 성능개선(2023 3.7.(화) ~ )

### 개요

다음의 목표를 달성하기 위해 과거 수행했던 프로젝트를 개선하는 작업을 2주간 진행

> 프로젝트에 대한 소개는 과거에 작성한 리드미 파일은 를 참조 바람

### 목표

- 제한된 자원(Core 1개, RAM 1GB)를 효율적으로 활용하기 위한 코드 리팩토링
- 파이썬 중급 사용자로서 도약(Coroutine, Pydantic, Logging, Cprofiler 등 경험)
- 클린 코드 / 문서화 / 테스트 / 프로파일링
- 빠른 구현 & 불필요한 기능에 집착하지 않기
- 가능하면 프론트엔드 건드리지 말기(만약 건드린다면 도서관 검색기능 처럼 구현)
...
- [ ] 컴퓨터/데이터 분야 도서 정보 확보 & 핵심데이터 추출 및 저장
- [ ] 웹크롤링을 통한 텍스트 데이터 확보 & 데이터 전처리

#### 모델 선정

- [ ] 사용자 검색 키워드와 연관성 높은 키워드 생성을 위해 W2V 모델 활용
- [ ] 도서를 대표하는 핵심 키워드 추출을 위해 ELECTRA 모델 활용

#### 모델 학습

- [ ] W2V : 컴퓨터/데이터 분야 키워드 간 관계 파악을 위한 모델 학습
- [ ] ELECTRA : 더 나은 키워드 추출을 위해 확보한 도서 데이터를 기반으로 Domain Adaptation 수행

#### 데이터 수집 및 학습 서버 개발

- [ ] 매월 도서 정보가 업데이트 되는 시점에 맞춰 데이터 수집 및 모델 학습을 자동화하는 서버 개발
- [ ] 제한적인 서버 자원(Cpu 1개, Ram 1GB)으로 인해 로컬 환경에서 서버 구동 및 업데이트

#### 웹 서버 개발

- [ ] 키워드 기반 검색 기능을 구현하기 위한 기초적인 방식의 검색 알고리즘 개발
  - [ ] Eleastic Search에서 키워드 기반 검색을 지원하는지 모르겠다. 가능하고 간단하다면 사용해보기
- [ ] 검색된 키워드와 사용자가 선택한 도서 정보간 관계를 확인할 수 있는 DashBoard 생성

### 관련 포스팅

- CPU 최적화 관련 포스팅(예정)

- Memory 최적화 관련 포스팅(예정)
