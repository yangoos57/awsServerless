...
class BookPipe:
    ...
    def _scrap_book_info_using_multiprocess(self, isbns: List[str]) -> List:
        ...
        for i, future in enumerate(as_completed(futures)):
            ...
            stringfied_result.append(result)
            if i % 50 == 0 and i != 0:
                self.logging.info(f"진행상태 : {i}/{len(futures)}")

        return list(map(literal_eval, stringfied_result))
    ...
    def extract_keywords_using_book_info(self, scraping_df: pd.DataFrame, worker: Callable) -> None:
        """수집한 데이터를 바탕으로 electra model을 활용해 도서 핵심 키워드 추출"""
        self.logging.info("<----extract keywords using book_info---->")

        dict_list: List[Dict] = self._extract_keywords_using_mutiprocess(scraping_df, worker)
    ...
    def _extract_keywords_using_mutiprocess(
        self, docs: pd.DataFrame, worker: Callable
    ) -> List[Dict]:
        ...
        for i in range(0, len(docs), chunk):
            ...
            future = executor.submit(worker, tasks)
            futures.append(future)
